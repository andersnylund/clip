import { closestCorners, getViewRect, KeyboardCode, RectEntry, KeyboardCoordinateGetter } from '@dnd-kit/core'
...
export const sortableTreeKeyboardCoordinates: (
  ...
  event,
  { active, currentCoordinates, context: { over, translatedRect, droppableContainers } }
) => {
  if (directions.includes(event.code)) {
    ...
    if (horizontal.includes(event.code) && over?.id) {
      const { depth, maxDepth, minDepth } = getProjection(items, active, over.id, offset, indentationWidth)
    ...
    const overRect = over?.id ? droppableContainers[over.id]?.rect.current : undefined
    ...
    if (closestId && over?.id) {
      ...
      if (newNode && activeNodeRect) {
        ...
        if (newItem && activeItem) {
          ...
          const offset =
            newRect.offsetTop > activeNodeRect.offsetTop ? Math.abs(activeNodeRect.height - newRect.height) : 0
